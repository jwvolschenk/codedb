// Trigram index — in-memory heap variant.
//
// Extracted from the former trigram.zig monolith. The mmap variant lives in
// mmap.zig and the union dispatcher lives in any.zig. trigram.zig re-exports
// all three so callers keep using a single import path.
