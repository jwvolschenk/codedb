    ...
    if (std.mem.eql(u8, cmd, "tree")) {
        ...
        for (outline.symbols.items) |sym| {
            ...
            if (sym.return_type) |rt| {
                out.p(" {s}->{s} {s}", .{ s.cyan, s.reset, rt });
            }
            if (sym.param_types.len > 0) {
                out.p(" {s}({s}", .{ s.dim, s.reset });
                for (sym.param_types, 0..) |pt, i| {
                    if (i > 0) out.p(", {s}", .{s.dim});
                    out.p("{s}", .{pt});
                }
                out.p("{s}){s}", .{ s.dim, s.reset });
            }
        ...
        if (try explorer.findSymbol(name, allocator)) |r| {
            defer {
                ...
                if (r.symbol.return_type) |rt| allocator.free(rt);
                for (r.symbol.param_types) |pt| allocator.free(pt);
                if (r.symbol.param_types.len > 0) allocator.free(r.symbol.param_types);
            }
            const elapsed = cio.nanoTimestamp() - t0;
            var dur_buf: [64]u8 = undefined;
            ...
            if (r.symbol.param_types.len > 0) {
                out.p("  {s}params:{s} ", .{ s.dim, s.reset });
