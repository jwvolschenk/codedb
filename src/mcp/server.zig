...
pub const tools_list =
    ...
    \\{"name":"codedb_callers","description":"Find call sites / references of a named symbol. For a type definition (class/interface/struct/enum) the default automatically broadens to whole-word references — type usages like List<T>, fields, params, base clauses — mirroring LSP findReferences. For functions/methods the default stays semantic invocation matching. Use match_mode=text for raw whole-word mentions (includes strings/comments).","inputSchema":{"type":"object","properties":{"name":{"type":"string","description":"Symbol name (exact identifier match)"},"max_results":{"type":"integer","description":"Maximum call sites to return (default: 50)"},"match_mode":{"type":"string","enum":["semantic","text","both","references"],"description":"semantic (default): invocation-looking sites; references: whole-word type usages excluding strings/comments (auto-selected for type definitions when mode is omitted); text: all whole-word mentions (incl. strings/comments); both: semantic OR text."},"include_generated":{"type":"boolean","description":"Include generated-file matches (default: false)"},"output_format":{"type":"string","enum":["text","json"],"description":"text (default) or json with confidence/why_matched metadata"},"project":{"type":"string","description":"Optional absolute path to a different project (must have codedb.snapshot)"}},"required":["name"]}},
    \\{"name":"codedb_relations","description":"First-class relation query for a symbol: definitions, inheritance, type/dependency users, and heuristic callers/references. For a type definition the default automatically broadens caller matching to whole-word references (mirroring LSP findReferences). Pass output_format=json for machine-friendly grouped JSON with confidence/why_matched metadata.","inputSchema":{"type":"object","properties":{"symbol":{"type":"string","description":"Symbol/type name to inspect"},"name":{"type":"string","description":"Alias for symbol"},"max_results":{"type":"integer","description":"Maximum caller search results to inspect (default: 50)"},"match_mode":{"type":"string","enum":["semantic","text","both","references"],"description":"Caller/reference matching mode. semantic (default): invocation-looking sites; references: whole-word type usages excluding strings/comments (auto-selected for type definitions when mode is omitted); text: broad whole-word mentions; both: semantic OR text."},"output_format":{"type":"string","enum":["text","json"],"description":"text (default) or json"},"project":{"type":"string","description":"Optional absolute path to a different project (must have codedb.snapshot)"}},"required":[]}},
