...
pub const tools_list =
    ...
    \\{"name":"codedb_callers","description":"Find call sites of a named symbol. Defaults to heuristic semantic invocation matching; use match_mode=text for broad whole-word mentions.","inputSchema":{"type":"object","properties":{"name":{"type":"string","description":"Symbol name (exact identifier match)"},"max_results":{"type":"integer","description":"Maximum call sites to return (default: 50)"},"match_mode":{"type":"string","enum":["semantic","text","both"],"description":"semantic (default) keeps invocation-looking sites; text is whole-word mentions; both maximizes recall."},"include_generated":{"type":"boolean","description":"Include generated-file matches (default: false)"},"project":{"type":"string","description":"Optional absolute path to a different project (must have codedb.snapshot)"}},"required":["name"]}},
